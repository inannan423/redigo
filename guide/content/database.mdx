...
在这里我们使用 Go 1.20 引入的 `Swap` 方法来原子地添加键值对。`Swap` 方法会返回被替换的旧值和一个布尔值表示键是否存在。这样可以避免 `Load` + `Store` 之间的竞态条件。
...
接下来实现 `PutIfAbsent` 和 `PutIfExists` 方法。`PutIfAbsent` 方法用于仅在键不存在时添加键值对，`PutIfExists` 方法用于仅在键存在时添加键值对。

<Callout type="warning">
**并发安全注意事项**

如果简单地使用 `Load` 方法来判断键是否存在，然后使用 `Store` 方法来添加键值对，会存在 **"check-then-act"（先检查后操作）** 竞态条件问题。

例如，在 `PutIfAbsent` 中：
- Goroutine A 调用 `Load(key)` 发现键不存在
- Goroutine B 也调用 `Load(key)` 发现键不存在
- Goroutine B 调用 `Store(key, valB)` 成功插入
- Goroutine A 调用 `Store(key, valA)` 覆盖了 B 的值

这违反了 "仅在不存在时插入" 的语义。因此我们需要使用 `sync.Map` 提供的原子操作来避免这个问题。
</Callout>

对于 `PutIfAbsent`，我们使用 `LoadOrStore` 方法。这个方法会原子地检查键是否存在，如果不存在则插入新值：
...
对于 `PutIfExists`，我们使用 Go 1.20 引入的 `CompareAndSwap` 方法。由于 `sync.Map` 没有直接提供 "仅在存在时更新" 的原子操作，我们需要使用 CAS（Compare-And-Swap）循环来实现：
...
这里使用了 CAS 循环：先加载旧值，然后尝试原子地替换。如果在 `Load` 和 `CompareAndSwap` 之间有其他 goroutine 修改了值，`CompareAndSwap` 会失败，我们就重试。

### Remove

接下来实现 `Remove` 方法，用于删除键值对。同样地，如果使用 `Load` + `Delete` 的方式，会存在竞态条件。我们使用 `LoadAndDelete` 方法（Go 1.15+）来原子地检查并删除键值对：
...
`LoadAndDelete` 方法会原子地删除键并返回被删除的值，避免了 `Load` 和 `Delete` 之间的竞态条件。
